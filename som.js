...
function startClock() {
  clockInterval = setInterval(function () {
    ...
    document.getElementById(
      "currentTime"
    ).innerText = `Current Time: ${hours}:${minutes}:${seconds}`;
  }, 1000);
}
...
function updateMoveCounter(isCorrect) {
  ...
  document.getElementById(
    "correctMoves"
  ).innerText = `Correct moves: ${correctMoves}`;
  document.getElementById(
    "correctCells"
  ).innerText = `Number of cells correct: ${correctCells}`;
  ...
  ).innerText = `Completion: ${completionPercentage.toFixed(2)}%`;
}
...
function validateInput(input) {
  ...
  if (/^[1-9]$/.test(value)) {
    ...
    if (boolTable[cellRow][cellCol]) {
      // Assuming boolTable indicates editable cells
...
function displaysubmit() {
  ...
  elapsedTimeDisplay.innerText = `Elapsed time: ${formatElapsedTime(
    elapsedTime
  )}`;
  ...
  document.getElementById(
    "movessubmit"
  ).innerText = `Number of moves: ${moves}`;
  document.getElementById(
    "correctMovessubmit"
  ).innerText = `Correct moves: ${correctMoves}`;
  document.getElementById(
  ...
  document.getElementById("accuracysubmit").innerText = `Accuracy: ${(moves ===
  0
    ? 0
    : (correctMoves / moves) * 100
  ).toFixed(2)}%`;
  document.getElementById(
    "completionPercentagesubmit"
  ).innerText = `Completion: ${(totalBlankCells === 0
    ? 0
    : (correctCells / totalBlankCells) * 100
  ).toFixed(2)}%`;
...
function logout() {
  ...
  document.getElementById("accuracy").innerText = `Accuracy: ${(moves === 0
    ? 0
    : (correctMoves / moves) * 100
  ).toFixed(2)}%`;
  document.getElementById(
